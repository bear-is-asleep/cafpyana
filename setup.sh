...
PYTHON_VERSION_ACTIVE=$(python -c 'import sys; print(f"{sys.version_info.major}.{sys.version_info.minor}")')

# Do not let spack's Python XRootD leak into the venv. It may be built for a
# different Python ABI, which causes pyxrootd.client imports to fail.
...
export LD_LIBRARY_PATH=${VENV_DIR}/lib/python3.10/site-packages/xrootd-5.6.9-py3.10-linux-x86_64.egg/pyxrootd:$LD_LIBRARY_PATH
export CAFPYANA_WD=${CAFPYANA_DIR}

# Get jobsub_submit from jobsub_lite
source /etc/profile.d/jobsub_lite.sh
which jobsub_submit   # should print /opt/jobsub_lite/bin/jobsub_submit
